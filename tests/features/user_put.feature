...
Feature: Check User[PUT] request
  ...
  Scenario: PUT user by username (successful) request with created username
    ...
    When I send "PUT" "user/username_test_put" request with json parameters:
      """
      {
        "id": 0,
        "username": "username_test_put_2",
    ...
    And I send "GET" "user/username_test_put_2" request
